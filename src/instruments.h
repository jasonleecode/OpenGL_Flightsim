...
inline void draw_map(ImDrawList* dl, const Style& style, const ImVec2& box_min, const ImVec2& box_max,
                     unsigned int texture, const glm::vec3& pos, float heading_deg, float terrain_size, float zoom,
                     const ImVec2& center_uv)
{
  const float margin = 12.0f;
  const ImVec2 map_min = add(box_min, ImVec2(margin, margin));
  const ImVec2 map_max = sub(box_max, ImVec2(margin, margin));

  // visible uv window around the given center, clamped so it stays inside the map
  const float half = 0.5f / zoom;
  const float u0   = glm::clamp(center_uv.x - half, 0.0f, 1.0f - 2.0f * half);
  const float v0   = glm::clamp(center_uv.y - half, 0.0f, 1.0f - 2.0f * half);
  ...
  // aircraft position: uv = pos.xz / terrain_size + 0.5 (same mapping as the terrain shader),
  // marker clamped to the map edge when the aircraft is outside the visible window
  const float u_ac = glm::clamp(pos.x / terrain_size + 0.5f, 0.0f, 1.0f);
  const float v_ac = glm::clamp(pos.z / terrain_size + 0.5f, 0.0f, 1.0f);
  const float u    = glm::clamp((u_ac - u0) / (2.0f * half), 0.0f, 1.0f);
  const float v    = glm::clamp((v_ac - v0) / (2.0f * half), 0.0f, 1.0f);
